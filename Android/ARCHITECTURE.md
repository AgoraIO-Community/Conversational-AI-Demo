...
| `agent-client-toolkit` | RTC/RTM 消息、字幕和指标 API | Maven `io.agora.agents:agora-agent-client-toolkit:2.10.1` |
...
- `io.agora.conversational.api`（Toolkit Maven 组件）
...
- Toolkit 与 `ui/living/legacy` 是字幕链路关键点，兼容性和包名结构都很敏感
...
Toolkit 作为 Maven `io.agora.agents:agora-agent-client-toolkit:2.10.1` 外部依赖下载；版本由 `gradle/libs.versions.toml` 管理，安装方式见 `scenes/convoai/README.md`。当前工程无需本地 Toolkit 子项目或源码路径。
...
- Toolkit 公共类型使用 `io.agora.conversational.api`，legacy v1 RTC stream renderer 保留在 demo
...
- 改 Toolkit 与 `ui/living/legacy`：验证 RTC/RTM、字幕更新、消息解析、回调派发、包名结构
...
4. [Toolkit 组件文档](https://github.com/AgoraIO-Conversational-AI/agent-client-toolkit-kotlin/blob/main/conversational-ai/README.md)
...
- Toolkit 接入细节：看 `scenes/convoai/README.md` 与 [Toolkit 组件文档](https://github.com/AgoraIO-Conversational-AI/agent-client-toolkit-kotlin/blob/main/conversational-ai/README.md)
