...
import io.agora.scene.convoai.ui.auth.SSOWebViewActivity
...
import io.agora.scene.convoai.ui.auth.CovPrivacyPolicyDialog
    ...
    private class SSOWebViewContract : ActivityResultContract<Int, String?>() {
        override fun createIntent(context: Context, input: Int): Intent {
            return Intent(context, SSOWebViewActivity::class.java).apply {
                putExtra(SSOWebViewActivity.Companion.EXTRA_TYPE, input)
            }
        }

        override fun parseResult(resultCode: Int, intent: Intent?): String? {
            return if (resultCode == RESULT_OK) {
                intent?.getStringExtra(SSOWebViewActivity.Companion.EXTRA_TOKEN)
            } else {
                null
            }
        }
    ...
    override fun initView() {
        ...
        mBinding?.apply {
            ...
            btnLoginSSO.setOnClickListener(object : OnFastClickListener() {
                override fun onClickJacking(view: View) {
                    if (cbTerms.isChecked) {
                        activityResultLauncher.launch(SSOWebViewActivity.Companion.TYPE_LOGIN)
                    } else {
                        animCheckTip()
                        showPrivacyDialog()
                    }
            ...
            btnSignUp.setOnClickListener(object : OnFastClickListener() {
                override fun onClickJacking(view: View) {
                    if (cbTerms.isChecked) {
                        activityResultLauncher.launch(SSOWebViewActivity.Companion.TYPE_SIGNUP)
                    } else {
                        animCheckTip()
                        showPrivacyDialog()
                    }
