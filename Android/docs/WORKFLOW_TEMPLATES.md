...
- 触及 `io.agora.conversational.api`（Toolkit Maven 组件） 或 `ui/living/legacy/` 字幕组件时，默认按高风险处理，扩大验证范围
...
- 是否涉及 Toolkit / `ui/living/legacy` 字幕组件、RTM 消息解析、字幕回调或包名结构？
...
- 重构目标和范围是什么？
- 哪些行为必须保持不变？
- 是否涉及公共基类、导航、Repository、并发、缓存、权限流程，或 Toolkit 字幕组件链路？
