...
- Product: Agora Convo AI Demo for Android. The primary UI stack is Android Views with ViewBinding.
...
- Build: the app currently has one `global` flavor and requires API 26 or newer. `app`, `common`, and `scenes:convoai` use Java 17; `iot` and `bleManager` use Java 11.
