...
- This repository currently has no version-controlled XCTest target or shared test scheme. Add or restore one before reporting a logic task as passed.
- Prefer XcodeBuildMCP test tools when available; otherwise use `xcodebuild test` through `Agent.xcworkspace` and a valid test scheme. Use `Agent-global` for app builds.
- Read `references/ios_logic_ut.md` before choosing a preflight, test-target, simulator, architecture, or retry path.
