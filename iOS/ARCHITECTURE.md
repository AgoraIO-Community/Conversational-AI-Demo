...
Agora Convo AI Demo for iOS demonstrates:
...
The application is built from `Agent.xcworkspace` with the `Agent-global` scheme and CocoaPods. The app target uses iOS 15.0 and Swift 5; no XCTest target is currently version-controlled.

```mermaid
flowchart LR
  app["Agent-global target\nAppDelegate, SceneDelegate, KeyCenter, resources"] --> common["Common local pod\nContext, network, user, logging, shared UI"]
...
|---|---|---|
| `Agent-global` / `Agent` | App lifecycle, root window, configuration injection, app resources, permissions, signing, versioning | Entry files: `AppDelegate.swift` and `SceneDelegate.swift` |
...
| XCTest target | Not currently version-controlled | Add or restore one before passing a logic change |
...
sequenceDiagram
  ...
  participant Transcript as "Toolkit transcript processor"
...
- Agent preset or REST changes: focused payload/DTO tests, optional fields, defaults, error paths, SIP branches, callers, and an appropriate test target.
...
The repository currently has no committed XCTest target. Add or restore one and run focused tests for logic work; a build-only result does not pass a logic acceptance criterion.
