...
- Workspace and targets: use `Agent.xcworkspace`, the `Agent-global` scheme, and CocoaPods. The app deployment target is iOS 15.0 and the project uses Swift 5.
...
- Tests: no XCTest target or shared test scheme is currently version-controlled. Logic work must add or restore an appropriate test target before it can pass acceptance.
...
- `.agents/skills/convoai-ios-workflow/references/ios_logic_ut.md`: focused logic-validation limits and the test-target gap.
